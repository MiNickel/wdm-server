...
export class FieldbookService {
  async getFieldbookForStation(stationId: string): Promise<any> {
    const stationData =
      query(`SELECT structure.name AS structure, station.name AS station, ms.querschnitt_block AS block, m.anfangs_datum AS date, m.taucher AS diver, m.protokollfuehrer AS "protocolLeader",
